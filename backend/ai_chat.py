...
def generate_ai_reply(user_text: str, history: list[dict] | None = None) -> str:
    """history: optional list of {"from": "user"|"ai", "text": str}, oldest
    first. Raises GeminiNotConfigured if no API key is set; raises on any
    other API-call failure. Callers must catch both and fall back — this
    function never returns a placeholder string on failure."""
